...
- OPC + COP + Tumor, only from ITAN-plasmid injected samples
...
```{r}
#| vscode: {languageId: r}
obj <- schard::h5ad2seurat(here('output/lgg_only/annotated_lgg_only_embedding.h5ad'))
obj
```
...
p1
ggsave(here('plot', 'Fig4D_lgg2.pdf'), p1, device = 'pdf', dpi = 300)
```
...
p2
ggsave(here('plot', 'Fig4D_lgg.pdf'), p2, device = 'pdf', dpi = 300)
```
